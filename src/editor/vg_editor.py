...
class VisualGraphWindow(QMainWindow):
    ...
    def initialize_filebar(self):
        # 左边是一个QFrame
        self.left_filebar = QWidget(self)
        self.left_filebar_layout = QHBoxLayout(self.left_filebar)
        self.left_filebar_layout.setContentsMargins(4, 4, 4, 4)

        # 添加文件浏览器
        file_sw = SidebarWidget(title='文件浏览器', isStretch=False)
        self.file_browser = FileBrowserWidget(file_sw)
        self.file_browser.fileSelected.connect(self.open_graph)
        file_sw.addComp('文件浏览器', self.file_browser, False, 3)
        self.left_filebar_layout.addWidget(file_sw)

    def initialize_sidebar(self):

        # 左边是一个QFrame
        self.left_sidebar = QWidget(self)
        self.left_layout = QHBoxLayout(self.left_sidebar)
    ...
    def setup_layout(self):
        ...
        self.splitter.addWidget(self.left_filebar)
