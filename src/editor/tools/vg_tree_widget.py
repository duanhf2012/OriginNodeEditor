...
class FileBrowserWidget(QTreeView):
    ...
    def __init__(self, parent=None):
        ...
        QSSLoadTool.setStyleSheetFile(self.tree_view, './src/editor/qss/tree.qss')
    ...
    def on_file_double_clicked(self, index):
        ...
        if os.path.isfile(file_path) and file_path.endswith('.vgf'):
            self.fileSelected.emit(file_path)

    def filter_files(self, text):
        if text:
            self.model.setNameFilters([f'*{text}*.vgf'])
        else:
            self.model.setNameFilters(['*.vgf'])
    #     # self.setColumnCount(1)
    #     self.setHeaderHidden(True)
    #     QSSLoadTool.setStyleSheetFile(self, './src/editor/qss/tree.qss')
    #
